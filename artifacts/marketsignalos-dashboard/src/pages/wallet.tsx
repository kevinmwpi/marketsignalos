...
export default function WalletPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setFailed(false);
    void getWalletDetail(address).then((result) => {
      if (!active) return;
      setDetail(result);
      setFailed(!result);
      setLoading(false);
      ...
      <div className="min-h-[100dvh] bg-[hsl(var(--background))]">
        <div className="mx-auto max-w-7xl space-y-4 px-6 py-8">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[1, 2, 3, 4].map((item) => <div className="h-20 animate-pulse rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))]" key={item} />)}
          </div>
          <div className="h-56 animate-pulse rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))]" />
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-[100dvh] bg-[hsl(var(--background))]">
        <main className="mx-auto max-w-4xl px-6 py-16">
          ...
          <div className="mt-4 flex items-center gap-4">
            <button className="text-sm font-semibold text-zinc-700 hover:underline" data-testid="button-retry-wallet" onClick={() => setAttempt((value) => value + 1)} type="button">
              Retry lookup
            </button>
    ...
    <div className="min-h-[100dvh] bg-[hsl(var(--background))]">
      <nav className="sticky top-0 z-10 border-b border-zinc-200 bg-white/95 backdrop-blur-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          <Link className="text-sm font-semibold tracking-tight text-zinc-900 hover:underline" href="/" data-testid="link-wallet-home">
            MarketSignalOS
          </Link>
          <a
