...
@pytest.mark.parametrize("path", [
    ...
    "/signals/fastlane/run", "/ingestor/watchlist",
])
...
def test_operator_logs_private_public_status_readable(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ADMIN_API_TOKEN", "secret")
    client = TestClient(create_app())
    for path in ("/ingestor/status", "/ingestor/watchlist", "/signals/notifications/status",
...
@pytest.mark.parametrize("summary", [
    ...
    {"windows_attempted": ["day", "week"], "windows_succeeded": ["day"]},
])
def test_collection_gaps_cannot_refresh_serving_success(
    tmp_path: Path, summary: dict[str, Any],
) -> None:
    _successful_run(tmp_path)
    success = load_receipt()["last_success_at"]
    save_receipt({"running": False, "last_exit_code": 0, "last_summary": summary,
                  "last_finished_at": "2099-01-01T00:00:00+00:00"})
    assert load_receipt()["last_success_at"] == success
    assert public_status()["freshness"] == "degraded"
