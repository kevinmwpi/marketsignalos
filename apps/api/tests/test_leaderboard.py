...
def test_leaderboard_returns_ranked_accounts(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    enrichment = [
        {
            "account_id": "acct-best",
            "as_of": "2026-03-02T00:30:00Z",
            "profile_confidence": 0.9,
            "cross_market_coordination": 0.8,
            "pre_resolution_accuracy": 0.7,
            "fill_intensity": 0.5,
        },
        {
            "account_id": "acct-mid",
    ...
    assert payload[0]["skill_score"] > payload[1]["skill_score"]


def test_leaderboard_returns_empty_when_files_missing(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("INGESTOR_FILLS_STORE_PATH", str(tmp_path / "missing-fills.jsonl"))
    monkeypatch.setenv(
        "INGESTOR_RESOLUTIONS_STORE_PATH", str(tmp_path / "missing-resolutions.jsonl")
    )
    monkeypatch.setenv(
        "INGESTOR_ACCOUNT_ENRICHMENT_STORE_PATH", str(tmp_path / "missing-enrichment.jsonl")
    )
