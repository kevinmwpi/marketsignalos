...
def _is_partial(summary: object) -> bool:
    if not isinstance(summary, dict):
        return False
    attempted = summary.get("windows_attempted")
    succeeded = summary.get("windows_succeeded")
    return bool(
        summary.get("warning") or summary.get("wallets_with_errors")
        or summary.get("enrichment_performed") is False
        or (isinstance(attempted, list) and attempted
            and set(attempted) != set(succeeded or []))
    )
